...
public class TestInMemoryEventStoreValue {
    ...
    public void testPutValueSameNameAndTime() {
        ...
        final InMemoryEventStoreValue store = new InMemoryEventStoreValue(
        ...
        for (final Entry<CellQualifier, ValueCellValue> entry : store) {
            assertEquals(10, entry.getValue().getAggregatedValue(), JUNIT_DOUBLE_ACCEPTABLE_DELTA);
        }
    }
    ...
    public void testPutValueSameNameDifferentTime() {
        ...
        for (final Entry<CellQualifier, ValueCellValue> entry : store) {
            assertEquals(0.6, entry.getValue().getAggregatedValue(), JUNIT_DOUBLE_ACCEPTABLE_DELTA);
        }
    }
    ...
    private void putCounts(final InMemoryEventStoreValue store, final RowKeyBuilder rowKeyBuilder, final double value,
