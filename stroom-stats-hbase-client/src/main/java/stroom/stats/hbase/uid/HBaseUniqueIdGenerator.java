...
public class HBaseUniqueIdGenerator implements UniqueIdGenerator {

    private static final Logger LOGGER = LoggerFactory.getLogger(HBaseUniqueIdGenerator.class);
    ...
    //Guice injection via UniqueIdGeneratorProvider
    public HBaseUniqueIdGenerator(final UniqueIdForwardMapTable forwardMapTable,
                                  final UniqueIdReverseMapTable reverseMapTable,
                                  final int width) {
        ...
        LOGGER.debug("Maximum UID for width {} is {}", width, maxId);
    }
    ...
    public byte[] getOrCreateId(final String name) {
        ...
        while (attempt-- > 0 && !bId.isPresent()) {
            ...
            if (!bId.isPresent()) {
                ...
                // save storage space
                final byte[] bNewId = UniqueIdGenerator.convertToUid(newId, width);
