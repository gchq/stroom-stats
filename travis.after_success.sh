...
if [ "$TRAVIS_EVENT_TYPE" = "cron" ]; then
    ...
    versionStr="${STROOM_STATS_VERSION}-${DATE_ONLY}-DAILY"
    ...
    #git push -q https://$TAGPERM@github.com/gchq/stroom-stats --follow-tags >/dev/null 2>&1
    #git push -q https://${TAGPERM}@github.com/${DOCKER_REPO} --follow-tags 
    doDockerBuild=true
...
if [ "$doDockerBuild" = true ] && [ "$TRAVIS_PULL_REQUEST" = "false" ] ; then
    ...
    docker build ${SPECIFIC_TAG} ${FLOATING_TAG} docker/.
...
#Deploy the generated swagger specs and swagger UI (obtained from github) to gh-pages
    ...
    #cp $TRAVIS_BUILD_DIR/stroom-stats-service/build/swagger/swagger.* $ghPagesDir/
    ...
    #sed -i 's#url: ".*"#url: "https://gchq.github.io/stroom-stats/swagger.json"#g' $ghPagesDir/index.html
#fi
